...
<head>
    ...
    <title>Edit Catatan Kasus</title>
...
<body>
    <div class="wrap">
        ...
        <h2>Edit Catatan Kasus — {{ $siswa->namlen ?? $siswa->nampan }}</h2>

        <form action="{{ route('guru.catatankasus.update', ['idkelas' => $kelas->id, 'id' => $catatan->id]) }}" method="POST">
            @csrf
            @method('PUT')

            <div class="field">
                <label>Dicatat oleh</label>
                <input type="text" value="{{ $guru->Nam ?? '-' }}" disabled>
            </div>

            <div class="field">
                <label>Tanggal</label>
                <input type="date" name="tanggal" value="{{ old('tanggal', \Carbon\Carbon::parse($catatan->tanggal)->format('Y-m-d')) }}" required>
                @error('tanggal') <span class="error-msg">{{ $message }}</span> @enderror
            </div>

            <div class="field">
                <label>Deskripsi Kasus</label>
                <textarea name="deskripsi_kasus" rows="5" required>{{ old('deskripsi_kasus', $catatan->deskripsi_kasus) }}</textarea>
                @error('deskripsi_kasus') <span class="error-msg">{{ $message }}</span> @enderror
            </div>

            <div class="field">
                <label>Jumlah Poin</label>
                <input type="number" name="jumlah_poin" min="0" value="{{ old('jumlah_poin', $catatan->jumlah_poin) }}" required>
