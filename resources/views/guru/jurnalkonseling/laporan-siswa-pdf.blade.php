...
<head>
    <meta charset="UTF-8">
    <title>Laporan Jurnal Konseling - {{ $siswaData->namlen ?? 'Siswa' }}</title>
    <style>
        @page {
...
<body>
    ...
    {{-- ===== INFO SISWA ===== --}}
    <table class="meta">
        <tr><td class="label">Nama Siswa</td><td class="colon">:</td><td>{{ $siswaData->namlen ?? '-' }}</td></tr>
        <tr><td class="label">NIS</td><td class="colon">:</td><td>{{ $siswaData->nis ?? '-' }}</td></tr>
    ...
    <table class="jurnal">
        ...
        <tbody>
            ...
            @empty
                <tr>
                    <td colspan="5" class="empty-note">Belum ada catatan jurnal konseling untuk siswa ini.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
